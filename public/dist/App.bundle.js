...
function ajaxVote(e) {
    ...
    _axios2.default.post(this.action, { chosenId: chosenId, pollId: pollId }).then(function (res) {
        ...
        (0, _bling.$)('.total-votes').textContent = res.data.total; // change the value in total
        // console.log($(`#${result[0].option}`))
        console.log(result);
        (0, _bling.$)('#' + result[0].option).children[1].innerHTML = result[0].votes; // change the value in the result option
        // and change the width of the result option
        var r = (0, _bling.$)('.results');
        r.querySelectorAll('.result').forEach(function (resl) {
            var votes = Number(resl.querySelector('.vote').innerHTML);
            var width = votes * 100 / res.data.total;
